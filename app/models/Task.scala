...
object Task extends DomainModel[Task] {
  ...
  def apply(title: String, description: String): Task = Task(0, title, description, false, null)
  def apply(id: Int, title: String, description: String, isDone: Boolean): Task =
    Task(id, title, description, isDone, null)
  def apply(id: Int): Task = Task(id, "", "", false, null)
}
